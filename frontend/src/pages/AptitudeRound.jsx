...
import { startAptitude, submitAptitude } from "../api/practice";
...
export default function AptitudeRound() {
  ...
  const [phase, setPhase] = useState("loading"); // loading | testing | submitting | feedback
  ...
  useEffect(() => {
    ...
    startAptitude({ role, mode, company })
      .then((data) => {
  ...
  const handleSubmit = async (isAutoSubmit = false) => {
    ...
    try {
      // Build responses from the *current* state right after flushing.
      ...
      const data = await submitAptitude(attemptId, { responses });
      setResult(data);
      setPhase("feedback");
    ...
    return (
      <div className="flex min-h-screen items-center justify-center bg-ink">
        <div className="flex items-center gap-3 text-sm text-text-muted">
          ...
          Generating your aptitude questions…
        </div>
      </div>
    );
  }
  ...
  if (phase === "feedback" && result) {
    return <AptitudeFeedback result={result} mode={mode} company={company} navigate={navigate} />;
  }
        ...
        <div className="grid grid-cols-1 gap-8 lg:grid-cols-[1fr_260px]">
          ...
          <div>
            <div className="flex items-center gap-3">
              ...
              <span className="text-xs text-text-faint">{q.subtype}</span>
            </div>
...
function AptitudeFeedback({ result, mode, company, navigate }) {
  ...
  return (
    <div className="min-h-screen bg-ink">
      ...
      <main className="mx-auto max-w-4xl px-6 py-16">
        <span className="tag-mono text-accent">Aptitude Round — complete</span>
        ...
        <div className="mt-10 panel p-6">
          <span className="tag-mono">Category breakdown</span>
